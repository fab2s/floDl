...
fn wizard(cli: &JoinConfigArgs) -> Result<Report, String> {
    ...
    let cloud_init_path = if cli.cloud_init {
        ...
        let content = render_cloud_init(&label, user, &worker_yml, &private_key);
...
/// The cloud-init user-data: worker yml, private key and the systemd
/// recipe embedded, so an instance boots straight into a persistent
/// `fdl join`. A SECRET artifact from the moment it exists — it carries
/// the key and the token — which is why it lands 0600 in the farm dir
/// and never on stdout.
///
...
/// option) then self-deprovisions the instance.
fn render_cloud_init(label: &str, user: &str, worker_yml: &str, private_key: &str) -> String {
    ...
    format!(
        "#cloud-config\n\
         ...
         # Assumes `fdl` is on the image (bake it in, or add a runcmd that\n\
         # installs it before the service starts).\n\
         write_files:\n\
         \x20 - path: /home/{user}/.ssh/flodl-join\n\
         \x20   owner: {user}:{user}\n\
         \x20   permissions: \"0600\"\n\
         \x20   defer: true\n\
         \x20   content: |\n{key}\n\
         \x20 - path: /home/{user}/training/fdl.yml\n\
         \x20   owner: {user}:{user}\n\
         ...
         \x20     Type=simple\n\
         \x20     User={user}\n\
         \x20     WorkingDirectory=/home/{user}/training\n\
