...
5. Writes the combined result under site/guide/<channel>/

Channels (see the block below for why):
  python3 site/build_guide.py                 unreleased -> site/guide/main/
                                              plus the bare-path redirects
  python3 site/build_guide.py --channel 0.8.x snapshot   -> site/guide/0.8.x/

Run before Jekyll: python3 site/build_guide.py  (or `make site`)
"""
...
GUIDE_ROOT = os.path.join(REPO_ROOT, "site", "guide")

# ── Channels ─────────────────────────────────────────────────────────────
#
# The guide publishes under a version segment, permanently: /guide/0.8.x/...
# for a release series and /guide/main/ for unreleased work. A bare
# /guide/... path is kept alive as a redirect to whichever series is current.
#
# The reason is link permanence, not readership. An unversioned URL whose
# content changes every release quietly re-points every link ever written to
# it, including ones in published posts and issues that cannot be edited. This
# is the shape Laravel, Rust and Django converged on for the same reason.
#
# A release series (X.Y.x) rather than a patch version: patch releases rarely
# move documentation, and a URL per patch multiplies permanent paths for
# nothing. A 0.8.1 doc fix lands in the same tree.
#
# Version trees are COMMITTED at release and never regenerated afterwards, so
# a typo in shipped docs can be fixed in place. That deliberately breaks
# reproducibility from the tag: see the README written into each tree.
MAIN_CHANNEL = "main"
SERIES_RE = re.compile(r"^\d+\.\d+\.x$")

# Set by main() from --channel. Everything the build writes goes under it.
GUIDE_DIR = None
...
def inject_channel(frontmatter, permalink, channel, is_current):
    """Point the page at its channel and tell the site how to treat it.

    The stub declares the bare permalink; the channel segment is added here so
    one stub serves every channel. `sitemap` carries the crawl decision: only
    the current series is listed, because every version is a near-duplicate of
    the others and listing them all spends crawl budget deciding which of five
    identical pages ranks. Superseded versions stay reachable and keep their
    inbound links; they simply stop competing.
    """
    lines = [ln for ln in frontmatter.splitlines() if not ln.startswith("permalink:")]
    body = "\n".join(ln for ln in lines if ln.strip() != "---")
    out = ["---", f"permalink: {permalink}", f"channel: {channel}"]
    if channel == MAIN_CHANNEL:
        out += ["noindex: true", "sitemap: false"]
    else:
        out += [f"sitemap: {'true' if is_current else 'false'}"]
        if not is_current:
            out += ["noindex: false"]
...
def sidebar_path(channel):
    """One sidebar per channel, and that is a correctness requirement.

    A single shared include would carry whichever channel built last, so a
    frozen 0.7.x tree would render main's navigation: links to pages that
    version never had, and missing ones it did. The nav has to freeze with the
    version it describes, exactly like the pages do.
    """
    return os.path.join(REPO_ROOT, "site", "_includes", f"sidebar-{channel}.html")
...
def render_sidebar(groups, permalinks, channel):
    ...
    out = [
        ...
        # Version switcher. Rendered from site/_data/guide_versions.yml rather
        # than baked in, so a series added later appears in every already-built
        # channel's nav without regenerating the frozen trees. Same-page
        # switching: the reader keeps their place across versions where the
        # page exists, which is the whole point of a switcher over a link home.
        '  {%- assign gv = site.data.guide_versions -%}',
        '  {%- if page.channel -%}',
        '  {%- assign tail = page.url | remove_first: "/guide/" '
        '| remove_first: page.channel | remove_first: "/" -%}',
        '  <div class="sidebar-versions">',
        '    <label for="guideVersion">Version</label>',
        '    <select id="guideVersion" '
        'onchange="if(this.value)window.location.href=this.value">',
        '      {%- for s in gv.series -%}',
        '      <option value="/guide/{{ s }}/{{ tail }}"'
        '{% if s == page.channel %} selected{% endif %}>'
        '{{ s }}{% if s == gv.current %} (current){% endif %}</option>',
        '      {%- endfor -%}',
        '      <option value="/guide/main/{{ tail }}"'
        '{% if page.channel == "main" %} selected{% endif %}>main (unreleased)</option>',
        '    </select>',
        '  </div>',
        '  {%- endif -%}',
    ]
    ...
    with open(sidebar_path(channel), "w") as f:
        f.write("\n".join(out))
    print(f"generated sidebar with {len(groups)} groups")


def guide_index_path():
    return os.path.join(GUIDE_DIR, "index.html")
...
def render_guide_index(groups, permalinks, channel, is_current):
    """Emit the channel's guide landing page, cards and all.
    ...
    The permalink carries the channel. It used to be a fixed `/guide/`, which
    every channel then claimed at once: Jekyll reported a destination conflict
    between the two channel landing pages and the bare-path redirect, picked
    one, and listed `/guide/` in the sitemap twice. A landing page per channel
    is the same rule the content pages follow.
    """
    out = [
        "---",
        "layout: guide",
        "title: Guide",
        f"permalink: /guide/{channel}/",
        f"channel: {channel}",
    ]
    if channel == MAIN_CHANNEL:
        out += ["noindex: true", "sitemap: false"]
...
def write_asset_hashes():
    """Content-hash everything under site/assets/ into _data/asset_hashes.yml.

    The stylesheet is served from a stable URL behind Cloudflare with a
    four-hour TTL, so a CSS change was invisible at the edge for up to four
    hours unless somebody purged by hand. That is how the version switcher
    shipped looking unstyled: the HTML was current, the stylesheet was the
    previous deploy's, and nothing about the page said so.

    A CONTENT hash rather than a build timestamp or a commit sha: those change
    on every deploy and would throw away a perfectly good edge cache each time,
    which is the opposite of the goal. This changes only when the bytes do.

    Query string rather than a hashed filename: Cloudflare and every CDN cache
    on the full URL, so `?v=<hash>` is a new object, and it needs no rename
    dance for files that are referenced from Liquid by their real path.
    """
    import hashlib

    assets = os.path.join(REPO_ROOT, "site", "assets")
    ...
    for dirpath, _dirs, files in os.walk(assets):
        for name in sorted(files):
            ...
            rel = "/" + os.path.relpath(path, os.path.join(REPO_ROOT, "site")).replace(os.sep, "/")
            with open(path, "rb") as f:
                digest = hashlib.sha256(f.read()).hexdigest()[:8]
            rows.append((rel, digest))
    rows.sort()

    os.makedirs(os.path.join(REPO_ROOT, "site", "_data"), exist_ok=True)
    with open(os.path.join(REPO_ROOT, "site", "_data", "asset_hashes.yml"), "w") as f:
        f.write("# Generated by site/build_guide.py. Do not edit.\n")
        f.write("# Cache-busting tokens: see write_asset_hashes() for why content, not time.\n")
...
def published_series():
    """Release series already published under site/guide/, newest last.

    Read off the filesystem rather than a declared version: what is CURRENT is
    a fact about what has been snapshotted, not about what Cargo.toml says. A
    tree that exists is served; one that does not cannot be linked to.
    """
    if not os.path.isdir(GUIDE_ROOT):
        return []
    found = [d for d in os.listdir(GUIDE_ROOT)
             if SERIES_RE.match(d) and os.path.isdir(os.path.join(GUIDE_ROOT, d))]
    return sorted(found, key=lambda s: [int(p) for p in s.split(".")[:2]])


def emit_redirects(current):
    """Write soft redirects at the bare /guide/... paths, pointing at `current`.

    Driven by the pages that EXIST in the current tree, never by the nav: a
    page present on main but not yet released would otherwise get a bare
    redirect aimed at a URL that 404s.

    These are per-page and enumerated on purpose. A wildcard rule
    (/guide/* -> /guide/<series>/$1) would also match /guide/<series>/... and
    send it to /guide/<series>/<series>/..., and it is the enumeration that
    makes that impossible rather than care.

    Soft redirects, because GitHub Pages cannot emit a 301. The canonical link
    is what consolidates ranking; the meta refresh is what moves a reader.
    """
    src_dir = os.path.join(GUIDE_ROOT, current)
    written, emitted = 0, set()
    for name in sorted(os.listdir(src_dir)):
        if not name.endswith(".md"):
            continue
        with open(os.path.join(src_dir, name)) as f:
            permalink = stub_field(f.read(), "permalink")
        if not permalink:
            continue
        bare = permalink.replace(f"/guide/{current}/", "/guide/", 1)
        # Belt and braces: a redirect that points at itself is a loop, and a
        # loop here would take out a page rather than merely mis-route it.
        if bare == permalink:
            print(f"error: refusing self-redirect for {permalink}", file=sys.stderr)
            sys.exit(1)
        with open(os.path.join(GUIDE_ROOT, name), "w") as f:
            f.write("---\n")
            f.write("layout: redirect\n")
            f.write(f"permalink: {bare}\n")
            f.write(f"redirect_to: {permalink}\n")
            f.write("sitemap: false\n")
            f.write("noindex: true\n")
            f.write("---\n")
        written += 1
        emitted.add(name)

    # The guide landing page moves with the rest. This one is tracked rather
    # than generated-and-ignored, because /guide/ is the most linked path on
    # the site and a build failure must not take it out.
    with open(os.path.join(GUIDE_ROOT, "index.html"), "w") as f:
        f.write("---\n")
        f.write("layout: redirect\n")
        f.write("permalink: /guide/\n")
        f.write(f"redirect_to: /guide/{current}/\n")
        f.write("sitemap: false\n")
        f.write("noindex: true\n")
        f.write("---\n")

    # A page dropped from the current series leaves a redirect behind that now
    # points at a 404. Prune, for the same reason the channel build prunes its
    # own leftovers: a stale generated file is a URL nobody decided to publish.
    for leftover in sorted(os.listdir(GUIDE_ROOT)):
        if leftover.endswith(".md") and leftover not in emitted:
            os.remove(os.path.join(GUIDE_ROOT, leftover))
            print(f"pruned stale redirect: site/guide/{leftover}")
    return written
...
def main():
    ...
    parser.add_argument(
        "--channel", default=MAIN_CHANNEL,
        help="main (default, unreleased) or a release series like 0.8.x to snapshot",
    )
    args = parser.parse_args()

    channel = args.channel
    if channel != MAIN_CHANNEL and not SERIES_RE.match(channel):
        print(f"error: --channel must be {MAIN_CHANNEL!r} or X.Y.x, got {channel!r}",
              file=sys.stderr)
        sys.exit(1)

    # A run writes its own channel directory and nothing else. This is what
    # keeps a released tree frozen: /guide/0.8.x stays 0.8.x forever, and no
    # later build can quietly re-point or regenerate it.
    global GUIDE_DIR
    GUIDE_DIR = os.path.join(GUIDE_ROOT, channel)

    # A snapshot being taken IS the new current, but re-running an older series
    # to patch its docs must not steal that status, so decide by comparing
    # against everything published including this run.
    known = sorted(set(published_series()) | ({channel} if channel != MAIN_CHANNEL else set()),
                   key=lambda s: [int(p) for p in s.split(".")[:2]])
    is_current = bool(known) and channel == known[-1]

    # Liquid cannot list a directory, so the channel set reaches the templates
    # as data. This drives both the version switcher and the banner that tells
    # a reader which version they landed on, which is the part that actually
    # matters: arriving on superseded docs from a search engine is the failure
    # mode versioned URLs create, and a banner is the fix for it.
    write_asset_hashes()

    os.makedirs(os.path.join(REPO_ROOT, "site", "_data"), exist_ok=True)
    with open(os.path.join(REPO_ROOT, "site", "_data", "guide_versions.yml"), "w") as f:
        f.write("# Generated by site/build_guide.py. Do not edit.\n")
        f.write(f"current: {known[-1] if known else ''}\n")
        f.write("series:\n")
        for s in reversed(known):
            f.write(f"  - {s}\n")
    ...
    for stub, title, group_label in chain:
        ...
        # The channel segment is added once, here: the sidebar, the guide
        # index, prev/next and the structural link rewrite all read this map,
        # so prefixing it is what makes every internal link stay inside its
        # own channel instead of leaking across versions.
        permalinks[stub] = permalink.replace("/guide/", f"/guide/{channel}/", 1)
    ...
    render_sidebar(groups, permalinks, channel)
    render_guide_index(groups, permalinks, channel, is_current)
    render_docs_index(groups, sources)

    print(f"generated {count} guide pages in site/guide/{channel}/")

    # Only the main build refreshes the bare-path redirects, and it aims them
    # at whatever series is published, so cutting a release is what moves
    # /guide/... onto the new version. Nothing here writes into a version tree.
    if channel != MAIN_CHANNEL:
        # The guard against a future contributor "fixing" the divergence.
        with open(os.path.join(GUIDE_DIR, "README.md"), "w") as f:
            f.write(f"""# flodl {channel} guide, frozen

Committed on purpose. This tree is what /guide/{channel}/ serves, and it keeps
serving it after `docs/` has moved on, so links written against {channel} do
not rot.

DO NOT REGENERATE THIS DIRECTORY. It was generated once at release, and it is
expected to diverge from `docs/` at the {channel} tag: a typo found in shipped
documentation is fixed by editing the file here, which is the point of
committing it. Regenerating from the tag would silently revert those fixes,
and regenerating from main would publish unreleased content under a released
version number.

To correct something here, edit it here. To change what future releases say,
edit `docs/` and it lands at the next snapshot.
""")

    if channel == MAIN_CHANNEL:
        series = published_series()
        if series:
            n = emit_redirects(series[-1])
            print(f"redirected {n} bare /guide/ paths to /guide/{series[-1]}/")
        else:
            print("note: no release series published yet, so no bare-path redirects")
