---
permalink: /guide/0.8.x/multi-gpu
channel: 0.8.x
sitemap: true
layout: guide
title: "Multi-GPU Training"
prev_url: /guide/0.8.x/graph-tree
prev_title: "Graph Tree"
next_url: /guide/0.8.x/async-ddp
next_title: "Heterogeneous & Multi-Host DDP"
anchors:
  - id: "the-one-liner"
    title: "The one-liner"
  - id: "what-just-happened---auto-promote"
    title: "What just happened - auto-promote"
  - id: "the-critical-invariant---no-cuda-before-trainerrun"
    title: "The critical invariant - no CUDA before Trainer::run"
  - id: "graph-models-the-same-entry"
    title: "Graph models: the same entry"
  - id: "pytorch-comparison"
    title: "PyTorch comparison"
  - id: "elche---heterogeneous-gpus-train-at-their-own-pace"
    title: "ElChe - heterogeneous GPUs train at their own pace"
  - id: "tuning-elche"
    title: "Tuning ElChe"
  - id: "mode-selection---elchemode"
    title: "Mode selection - ElCheMode"
  ...
  - id: "scaling-out---multi-host-clusters"
    title: "Scaling out - multi-host clusters"
  - id: "resume--checkpoints"
    title: "Resume + checkpoints"
  - id: "manual-control---ddpwrap-expert-bypass"
    title: "Manual control - Ddp::wrap (expert bypass)"
  - id: "quick-reference"
...
> **Prerequisites**: [Training](/guide/0.8.x/training) covers single-device
> training. This tutorial assumes the universal `Trainer::builder`
> shape - extending it to N GPUs is configuration, not code.

> **Time**: ~20 minutes.

> **Canonical reference**: [DDP Reference](/guide/0.8.x/ddp/reference) for the full
> knob surface.

> **Runnable example**: [`auto_promote`](https://github.com/flodl-labs/flodl/tree/main/flodl/examples/auto_promote) is the
...
[HuggingFace Integration](/guide/0.8.x/flodl-hf). To keep the loop body
...
`overhead_target`](/guide/0.8.x/ddp/reference#guard-authority-over-overhead_target).
...
Reference](/guide/0.8.x/ddp/reference#elcheconfig-knobs)):
...
| `NcclSync` | Tightest cadence (reduce per slow-rank step, equal data split — not per-batch lockstep; see [What "sync" means](/guide/0.8.x/ddp/reference#elchemode---cadence--backend-in-one-name)) - homogeneous rigs, correctness-first baseline |
| `CpuSync`, `CpuCadence` | A/B against NCCL when peer-access is unavailable |

See [A/B testing modes](/guide/0.8.x/ddp/internals#ab-testing-modes) for the suggested
order and rationale.
...
See [Data Loading](/guide/0.8.x/data-loading) for the full
DataLoader surface.
...
`EpochCallbackPolicy`](/guide/0.8.x/ddp/reference#epochcallbackpolicy).
...
[Training Monitor](/guide/0.8.x/monitor) for the full surface.
...
clusters](/guide/0.8.x/ddp/cluster-guide) and [CLI Reference: cluster
commands](/guide/0.8.x/cli/install) for the full surface.
...
[DDP Reference: Resume + checkpoints](/guide/0.8.x/ddp/reference#resume--checkpoints).
