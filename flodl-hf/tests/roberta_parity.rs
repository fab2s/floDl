//! PyTorch parity for `RobertaModel::from_pretrained("FacebookAI/roberta-base")`.
//!
...
//!
//! `FacebookAI/roberta-base` doesn't ship pooler weights — RoBERTa pretraining
...
//!
//! `_live` — pulls the real `FacebookAI/roberta-base` weights from the Hub.
...
fn roberta_parity_vs_pytorch_live() {
    ...
    eprintln!(
        "FacebookAI/roberta-base last_hidden_state max_abs_diff = {diff:.3e} (tol {HIDDEN_TOL:.0e})"
    );
    assert!(
        diff <= HIDDEN_TOL,
