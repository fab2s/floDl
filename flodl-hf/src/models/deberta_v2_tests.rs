...
fn from_json_str_rejects_share_att_key_false() {
    let json = v3_base_config_json().replace("\"share_att_key\": true", "\"share_att_key\": false");
    let err = DebertaV2Config::from_json_str(&json)
        .unwrap_err()
        .to_string();
...
fn from_json_str_rejects_missing_p2c() {
    let json =
        v3_base_config_json().replace("\"pos_att_type\": \"p2c|c2p\"", "\"pos_att_type\": \"c2p\"");
    let err = DebertaV2Config::from_json_str(&json)
        .unwrap_err()
        .to_string();
...
fn from_json_str_rejects_token_types() {
    let json = v3_base_config_json().replace("\"type_vocab_size\": 0", "\"type_vocab_size\": 2");
    let err = DebertaV2Config::from_json_str(&json)
        .unwrap_err()
        .to_string();
...
fn backbone_has_no_absolute_position_or_token_type_embeddings() {
    ...
    for p in &expected {
        assert!(
            ...
            "v3 has no absolute position embeddings; got {}",
            p.key,
        );
        assert!(
            !p.key.contains("token_type_embeddings"),
            "v3 has no token-type embeddings; got {}",
            p.key,
        );
    }
}
...
fn mlm_head_ties_weight_and_emits_separate_bias() {
    ...
    assert!(
        !keys
            .iter()
            .any(|k| k.contains("lm_predictions.lm_head.decoder")),
