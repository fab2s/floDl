...
fn expected_layer_keys(i: i64) -> Vec<String> {
    ...
    suffixes
        .iter()
        .map(|s| format!("bert.encoder.layer.{i}.{s}"))
        .collect()
}
...
fn bert_parameter_shapes_match_bert_base_uncased() {
    ...
    assert_eq!(
        by_key["bert.embeddings.word_embeddings.weight"],
        &[30522, 768]
    );
    assert_eq!(
        by_key["bert.embeddings.position_embeddings.weight"],
        &[512, 768]
    );
    assert_eq!(
        by_key["bert.embeddings.token_type_embeddings.weight"],
        &[2, 768]
    );
    ...
    for i in 0..config.num_hidden_layers {
        ...
        assert_eq!(
            by_key[&*format!("{p}.attention.self.query.weight")],
            &[768, 768]
        );
        assert_eq!(by_key[&*format!("{p}.attention.self.query.bias")], &[768]);
        assert_eq!(
            by_key[&*format!("{p}.attention.self.key.weight")],
            &[768, 768]
        );
        assert_eq!(
            by_key[&*format!("{p}.attention.self.value.weight")],
            &[768, 768]
        );
        assert_eq!(
            by_key[&*format!("{p}.attention.output.dense.weight")],
            &[768, 768]
        );
        assert_eq!(
            by_key[&*format!("{p}.attention.output.LayerNorm.weight")],
            &[768]
        );
        assert_eq!(
            by_key[&*format!("{p}.intermediate.dense.weight")],
            &[3072, 768]
        );
...
fn bert_layer_count_scales_with_config() {
    for n in [1_i64, 3, 6] {
        ...
        let last_layer_key = format!("bert.encoder.layer.{}.attention.self.query.weight", n - 1,);
        assert!(
...
fn bert_config_from_json_str_rejects_missing_field() {
    ...
    assert!(
        err.contains("missing required integer field"),
...
fn bert_config_rejects_non_contiguous_id2label() {
    ...
    let err = BertConfig::from_json_str(json).unwrap_err().to_string();
    assert!(
        err.contains("contiguous"),
        "error must call out contiguity: {err}"
    );
}
...
fn bert_config_to_json_str_preserves_task_head_metadata() {
    ...
    assert_eq!(
        r.id2label.as_deref(),
        Some(&["POS".to_string(), "NEG".to_string(), "NEU".to_string(),][..])
    );
...
fn extended_attention_mask_shape_and_values() {
    ...
    assert!(
        (values[5] - -1e4).abs() < 1e-3,
        "masked position should be ~-1e4, got {}",
        values[5]
    );
}
// ── Task head tests ──────────────────────────────────────────────────
...
fn sequence_classification_labels_from_config_or_fallback() {
    ...
    assert_eq!(
        fallback.labels(),
        &["LABEL_0".to_string(), "LABEL_1".to_string()]
    );
}
...
fn question_answering_forward_shape_smoke() {
    ...
    let tt = Variable::new(
        Tensor::from_i64(&[0; 4], &[batch, seq], dev).unwrap(),
        false,
    );
    let mask_flat = Tensor::ones(
        &[batch, seq],
...
fn has_graph_returns_inner_graph_by_reference() {
    ...
    assert!(std::ptr::eq(
        seq.graph(),
        <BertForSequenceClassification as HasGraph>::graph(&seq)
    ));
    assert!(std::ptr::eq(
        token.graph(),
        <BertForTokenClassification as HasGraph>::graph(&token)
    ));
    assert!(std::ptr::eq(
        qa.graph(),
        <BertForQuestionAnswering as HasGraph>::graph(&qa)
    ));
}
...
fn masked_lm_has_graph_returns_inner_graph_by_reference() {
    ...
    assert!(std::ptr::eq(
        head.graph(),
        <BertForMaskedLM as HasGraph>::graph(&head)
    ));
}
...
fn mlm_loader_tolerates_redundant_tied_decoder_key() {
    ...
    let unused = load_safetensors_into_graph_with_rename_allow_unused(head.graph(), &bytes, |k| {
        k.to_string()
    })
    ...
    //    tolerance is opt-in.
    let strict_err =
        load_safetensors_into_graph_with_rename(head.graph(), &bytes, |k| k.to_string())
