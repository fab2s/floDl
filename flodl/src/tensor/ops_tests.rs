...
fn test_scalar_ops() {
    ...
    assert_eq!(
        t.add_scalar(1.0).unwrap().to_f32_vec().unwrap(),
        vec![3.0, 5.0]
    );
    assert_eq!(
        t.mul_scalar(3.0).unwrap().to_f32_vec().unwrap(),
        vec![6.0, 12.0]
    );
...
fn test_activations() {
    ...
    assert!(
        close && !exact,
        "erf vs tanh GELU: erf={g_erf:?}, tanh={g_tanh:?}"
    );
}
...
fn test_sign_floor_ceil_round() {
    ...
    assert_eq!(
        t.sign().unwrap().to_f32_vec().unwrap(),
        vec![-1.0, 0.0, 1.0]
    );
    assert_eq!(
        t.floor().unwrap().to_f32_vec().unwrap(),
        vec![-3.0, 0.0, 1.0]
    );
    assert_eq!(
        t.ceil().unwrap().to_f32_vec().unwrap(),
        vec![-2.0, 0.0, 2.0]
    );
...
fn test_maximum_minimum() {
    ...
    assert_eq!(
        a.maximum(&b).unwrap().to_f32_vec().unwrap(),
        vec![4.0, 5.0, 3.0]
    );
    assert_eq!(
        a.minimum(&b).unwrap().to_f32_vec().unwrap(),
        vec![1.0, 2.0, 3.0]
    );
}
