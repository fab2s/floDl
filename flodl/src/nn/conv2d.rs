...
mod tests {
    ...
    fn test_conv2d_forward() {
        ...
        let x = Variable::new(Tensor::randn(&[1, 3, 32, 32], test_opts()).unwrap(), false);
    ...
    fn test_conv2d_no_bias() {
        ...
        let x = Variable::new(Tensor::randn(&[2, 3, 16, 16], test_opts()).unwrap(), false);
        let y = conv.forward(&x).unwrap();
    ...
    fn test_conv2d_with_padding() {
        let conv = Conv2d::configure(1, 4, 3)
            ...
            .done()
            .unwrap();
        let x = Variable::new(Tensor::randn(&[1, 1, 8, 8], test_opts()).unwrap(), false);
        let y = conv.forward(&x).unwrap();
    ...
    fn test_conv2d_stride() {
        let conv = Conv2d::configure(1, 4, 3)
            ...
            .done()
            .unwrap();
        let x = Variable::new(Tensor::randn(&[1, 1, 16, 16], test_opts()).unwrap(), false);
        let y = conv.forward(&x).unwrap();
        assert_eq!(y.shape(), vec![1, 4, 8, 8]);
    }
    ...
    fn test_conv2d_gradient() {
        ...
        let x = Variable::new(Tensor::randn(&[2, 3, 8, 8], test_opts()).unwrap(), true);
    ...
    fn test_conv2d_grouped() {
        let conv = Conv2d::configure(4, 8, 3)
            ...
            .done()
            .unwrap();
        let x = Variable::new(Tensor::randn(&[1, 4, 10, 10], test_opts()).unwrap(), false);
        let y = conv.forward(&x).unwrap();
    ...
    fn test_conv2d_invalid_groups_error() {
        ...
        assert!(
            m.contains("in_channels (4) must be divisible by groups (3)"),
            "{m}"
        );

        let m = msg(Conv2d::configure(4, 6, 3).with_groups(4).done());
        assert!(
            m.contains("out_channels (6) must be divisible by groups (4)"),
            "{m}"
        );
    }
    ...
    fn test_conv2d_builder_without_bias() {
        let conv = Conv2d::configure(3, 16, 3).without_bias().done().unwrap();
        assert_eq!(conv.parameters().len(), 1);
