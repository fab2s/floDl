...
fn test_compressed_roundtrip() {
    ...
    for ((_, src), (_, dst)) in params.iter().zip(load_params.iter()) {
        assert_eq!(
            src.variable.data().to_f32_vec().unwrap(),
...
fn test_migrate_all_renamed() {
    ...
    let old_params = vec![
        (
            "linear_0/weight".to_string(),
            Parameter::new(
                Tensor::randn(&[4, 8], crate::tensor::test_opts()).unwrap(),
                "weight",
            ),
        ),
        (
            ...
            Parameter::new(
                Tensor::randn(&[8, 2], crate::tensor::test_opts()).unwrap(),
                "weight",
            ),
        ),
    ];
    ...
    let mut out = Vec::new();
    let report =
        migrate_checkpoint(&mut std::io::Cursor::new(&ckpt), &mut out, &new_params, &[]).unwrap();
...
fn test_migrate_partial_rename() {
    ...
    assert_eq!(
        report.remapped[0],
        ("linear_0/weight".to_string(), "encoder/weight".to_string())
    );
    assert!(report.is_complete());
}
...
fn test_migrate_dropped_and_missing() {
    let old_params = vec![
        ...
        (
            "removed/weight".to_string(),
            Parameter::new(
                Tensor::randn(&[16, 16], crate::tensor::test_opts()).unwrap(),
                "weight",
            ),
        ),
    ];
    ...
    let new_params = vec![
        ...
        (
            "added/weight".to_string(),
            Parameter::new(
                Tensor::randn(&[32, 32], crate::tensor::test_opts()).unwrap(),
                "weight",
            ),
        ),
    ];
...
fn test_migrate_positional_disambiguation() {
    ...
    let old_params = vec![
        (
            "linear_0/weight".to_string(),
            Parameter::new(
                Tensor::randn(&[4, 4], crate::tensor::test_opts()).unwrap(),
                "weight",
            ),
        ),
        (
...
fn test_migrate_file_roundtrip() {
    ...
    let load_report = load_checkpoint_file(dst.to_str().unwrap(), &vp, &[], None).unwrap();
...
fn test_migrate_file_in_place() {
    ...
    let path =
        std::env::temp_dir().join(format!("test_migrate_in_place_{}.fdl", std::process::id()));
...
fn test_load_rejects_lying_headers_without_allocating() {
    ...
    let msg = |buf: &[u8]| match load_checkpoint(&mut std::io::Cursor::new(buf), &[], &[], None) {
...
fn test_truncated_checkpoint_header_only() {
    ...
    assert!(
        result.is_err(),
        "truncated checkpoint should return Err, not panic"
    );
    let msg = format!("{}", result.unwrap_err());
...
fn test_zero_entries_nonempty_model() {
    ...
    assert_eq!(
        report.missing.len(),
        1,
        "model param should be reported as missing"
    );
}
...
fn test_shape_mismatch_transposed() {
    ...
    assert!(
        result.is_err(),
        "transposed shape should be a mismatch error"
    );
    let msg = format!("{}", result.unwrap_err());
    assert!(msg.contains("shape mismatch"), "error: {}", msg);
    assert!(
        msg.contains("[4, 8]"),
        "should show checkpoint shape: {}",
        msg
    );
...
fn test_dtype_mismatch_auto_cast() {
    ...
    let f32_param = vec![(
        "layer/weight".to_string(),
        Parameter::new(
            Tensor::ones(&[2, 3], crate::tensor::test_opts()).unwrap(),
            "weight",
        ),
    )];
    let mut buf = Vec::new();
...
fn test_dtype_mismatch_buffer_auto_cast() {
    ...
    let f32_buffers = vec![(
        "norm/running_mean".to_string(),
        Buffer::new(
            Tensor::ones(&[8], crate::tensor::test_opts()).unwrap(),
            "running_mean",
        ),
    )];
    let mut buf = Vec::new();
...
fn test_atomic_save_commits_and_leaves_no_tmp() {
    ...
    let report = load_checkpoint_file(path_str, &load_params, &load_buffers, None).unwrap();
    assert_eq!(report.loaded.len(), 2); // 1 param + 1 buffer
