...
impl ConvTranspose3dBuilder {
    ...
    pub fn with_output_padding(mut self, output_padding: [i64; 3]) -> Self {
...
mod tests {
    ...
    fn test_conv_transpose3d_forward() {
        ...
        let x = Variable::new(Tensor::randn(&[1, 4, 4, 4, 4], test_opts()).unwrap(), false);
    ...
    fn test_conv_transpose3d_gradient() {
        let conv = ConvTranspose3d::on_device(4, 1, [3, 3, 3], test_device()).unwrap();
        let x = Variable::new(Tensor::randn(&[1, 4, 4, 4, 4], test_opts()).unwrap(), true);
