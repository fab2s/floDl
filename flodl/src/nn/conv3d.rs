...
impl Conv3d {
    ...
    pub fn new(in_channels: i64, out_channels: i64, kernel_size: [i64; 3]) -> Result<Self> {
        Self::build(
            in_channels,
            out_channels,
            kernel_size,
...
mod tests {
    ...
    fn test_conv3d_forward() {
        ...
        let x = Variable::new(Tensor::randn(&[1, 1, 8, 8, 8], test_opts()).unwrap(), false);
    ...
    fn test_conv3d_gradient() {
        ...
        let x = Variable::new(Tensor::randn(&[1, 1, 6, 6, 6], test_opts()).unwrap(), true);
    ...
    fn test_conv3d_no_bias() {
        ...
        let x = Variable::new(Tensor::randn(&[1, 1, 6, 6, 6], test_opts()).unwrap(), false);
        let y = conv.forward(&x).unwrap();
    ...
    fn test_conv3d_with_padding() {
        let conv = Conv3d::configure(1, 2, [3, 3, 3])
            ...
            .done()
            .unwrap();
        let x = Variable::new(Tensor::randn(&[1, 1, 4, 4, 4], test_opts()).unwrap(), false);
        let y = conv.forward(&x).unwrap();
