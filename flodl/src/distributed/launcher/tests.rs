use super::spawn::{
    PerHostPrebuild, build_remote_agent_bash_command, build_slim_envelope_for, shell_quote,
    supervise_children,
};
use super::*;
use serde_json::json;
...
fn agent_bash_command_exports_agent_env_only() {
    ...
    assert!(
        cmd.contains("FLODL_HOST_NAME='pascal'"),
        "missing host override: {cmd}"
    );
    ...
    assert!(
        !cmd.contains("FLODL_INTERNAL_CLUSTER_JSON="),
        "leaked rank envelope: {cmd}"
    );
    assert!(
        !cmd.contains("FLODL_INTERNAL_LOCAL_RANK="),
        "leaked rank slot: {cmd}"
    );
    assert!(
        !cmd.contains("FLODL_INTERNAL_RELAY_JSON="),
        "leaked relay spec: {cmd}"
    );
    assert!(
        !cmd.contains("CUDA_VISIBLE_DEVICES="),
        "agent must not scope CUDA: {cmd}"
    );
    // Trap wrapper for clean signal forwarding.
    assert!(
        cmd.contains("trap ") && cmd.contains("__flodl_pid"),
        "missing trap: {cmd}"
    );
}
...
fn synthesized_world_reranks_config_hosts_and_admits_walk_ins() {
    ...
    assert_eq!(
        world.workers[0].nccl_socket_ifname,
        cfg_b.nccl_socket_ifname
    );
...
fn rejects_empty_workers() {
    ...
    assert!(
        err.to_string().contains("workers must be non-empty"),
        "got: {err}"
    );
}
...
fn data_path_survives_full_cluster_to_json() {
    ...
    let round = FullCluster::from_value(&FullCluster::from_value(&v).unwrap().to_json()).unwrap();
...
fn gpu_ram_share_resolves_host_over_cluster_default_in_the_envelope() {
    ...
    assert_eq!(
        a.gpu_ram_share(),
        Some(0.25),
        "the host declaration must win"
    );
    assert_eq!(
        b.gpu_ram_share(),
        Some(0.5),
        "a silent host inherits the default"
    );
    ...
    v["workers"][0]
        .as_object_mut()
        .unwrap()
        .remove("gpu_ram_share");
    let full = FullCluster::from_value(&v).unwrap();
    let env = build_slim_envelope_for(&full, &full.workers[0], &full.controller.host, false);
...
fn gpu_ram_share_refuses_negatives_but_keeps_above_one_legal() {
    ...
    assert_eq!(
        FullCluster::from_value(&v).unwrap().gpu_ram_share,
        Some(1.2)
    );
}
...
fn slim_envelope_rank_resources_round_trips() {
    ...
    assert!(
        off.get("rank_resources").is_none(),
        "off run must not emit the key"
    );
...
fn tunnel_field_parses_defaults_false_and_round_trips() {
    ...
    let err = FullCluster::from_value(&v).unwrap_err();
    assert!(
        err.to_string().contains("tunnel must be a boolean"),
        "got: {err}"
    );
}
...
fn agent_bash_command_prebuild_yields_to_host_env_ld_path() {
    ...
    let host_pos = s
        .find("LD_LIBRARY_PATH='/opt/libtorch/lib:/usr/local/lib'")
        .unwrap();
...
fn agent_bash_command_exports_cluster_and_host_env() {
    ...
    assert!(
        cluster_pos < host_pos,
        "host env must export after cluster env"
    );
}
...
fn slim_envelope_propagates_session_salt() {
    ...
    let salt: crate::distributed::wire::SessionSalt = [
        0xde, 0xad, 0xbe, 0xef, 0x01, 0x02, 0x03, 0x04, 0xfe, 0xed, 0xfa, 0xce, 0x05, 0x06, 0x07,
        0x08,
    ];
...
fn worker_env_block_rejects_reserved_keys() {
    let mut val = canonical_full_json();
    val["workers"][0]["env"] = serde_json::json!({ "FLODL_INTERNAL_CLUSTER_JSON": "deadbeef" });
...
fn supervise_elastic_tolerates_death_and_reports_ranks() {
    ...
    let queue: ReportedDeaths = std::sync::Arc::new(std::sync::Mutex::new(Vec::new()));
    ...
    let elastic = ElasticSupervision {
        ...
        max_failure: Some(crate::distributed::max_failure::MaxFailureThreshold::Absolute(2)),
        world_size: 2,
        cohort_formed: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(true)),
    };
...
fn supervise_elastic_pre_formation_kills_all() {
    ...
    let elastic = ElasticSupervision {
        ...
        max_failure: None,
        world_size: 2,
        cohort_formed: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false)),
    };
    let start = std::time::Instant::now();
...
fn supervise_elastic_verdict_fails_past_threshold() {
    ...
    assert!(
        err.to_string().contains("max_failure exceeded"),
        "got: {err}"
    );
}
