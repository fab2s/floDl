use super::*;
use crate::distributed::cluster_coordinator::{ClusterCoordinator, ClusterCoordinatorConfig};
...
fn spawn_coord<D>(world_size: usize, drive: D) -> (u16, thread::JoinHandle<Result<()>>)
where
...
fn end_to_end_sync_nccl_via_coord_smoke() {
    ...
    let devices: Vec<Device> = (0..world_size as u8).map(Device::CUDA).collect();
...
impl crate::distributed::ddp_run::convergence::ConvergenceGuard for RecordingGuard {
    fn clone_box(&self) -> Box<dyn crate::distributed::ddp_run::convergence::ConvergenceGuard> {
...
fn end_to_end_sync_cpu_smoke() {
    ...
    let worker_results: Vec<(usize, std::thread::Result<Result<()>>)> = worker_handles
        .into_iter()
        .enumerate()
