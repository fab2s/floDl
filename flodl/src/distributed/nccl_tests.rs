...
fn test_nccl_broadcast() {
    ...
    assert!(
        vals0.iter().all(|&v| (v - 42.0).abs() < 1e-5),
        "device 0 should still have 42.0"
    );
    assert!(
        vals1.iter().all(|&v| (v - 42.0).abs() < 1e-5),
...
fn test_nccl_all_reduce_sum() {
    ...
    let vals1 = t1.to_f32_vec().unwrap();
    assert!(
        vals0.iter().all(|&v| (v - 3.0).abs() < 1e-5),
        "device 0 should have 3.0 after AllReduce Sum"
    );
    assert!(
        vals1.iter().all(|&v| (v - 3.0).abs() < 1e-5),
...
fn test_nccl_all_reduce_avg() {
    ...
    let vals1 = t1.to_f32_vec().unwrap();
    assert!(
        vals0.iter().all(|&v| (v - 15.0).abs() < 1e-5),
        "device 0 should have 15.0 after AllReduce Avg"
    );
    assert!(
        vals1.iter().all(|&v| (v - 15.0).abs() < 1e-5),
...
fn test_nccl_all_reduce_on_streams() {
    ...
    comms
        .all_reduce_on_streams(&[&t0, &t1], ReduceOp::Sum, &[&stream0, &stream1])
        .unwrap();
    ...
    let vals1 = t1.to_f32_vec().unwrap();
    assert!(
        vals0.iter().all(|&v| (v - 12.0).abs() < 1e-5),
        "device 0 should have 12.0 after AllReduce Sum on streams"
    );
    assert!(
        vals1.iter().all(|&v| (v - 12.0).abs() < 1e-5),
...
fn test_nccl_rank_comm_rejects_invalid_rank() {
    let result = NcclRankComm::init_rank(
        2,
        2,
        &NcclUniqueId {
            bytes: [0; NCCL_UNIQUE_ID_BYTES],
        },
    );
...
fn test_nccl_rank_comm_premul_sum_weighted_consensus() {
    ...
    assert!(
        (v0 - 12.5).abs() < 1e-5,
        "rank0 consensus should be 12.5, got {v0}"
    );
    assert!(
        (v1 - 12.5).abs() < 1e-5,
        "rank1 consensus should be 12.5, got {v1}"
    );
}
...
fn test_nccl_weighted_allreduce_syncs_buffers_mover_averaged() {
    ...
    let run =
        |comm: crate::distributed::nccl::NcclRankComm, dev: u8, p: Tensor, b: Tensor, n_i: f64| {
    ...
    gpu_synchronize(1);
    for (t, want, what) in [
        (&p0, 12.5, "rank0 params"),
        (&p1, 12.5, "rank1 params"),
        (&b0, 150.0, "rank0 buffers"),
        (&b1, 150.0, "rank1 buffers"),
    ] {
    ...
    for (t, want, what) in [
        (&p0, 1.0, "rank0 params"),
        (&p1, 1.0, "rank1 params (adopted)"),
        (&b0, 5.0, "rank0 buffers"),
        (&b1, 5.0, "rank1 buffers (adopted)"),
    ] {
        let v: f64 = t.mean().unwrap().item().unwrap();
        assert!((v - want).abs() < 1e-4, "{what}: want {want}, got {v}");
    }
...
fn test_nccl_rank_comm_init_and_reduce() {
    ...
    assert!(
        vals0.iter().all(|&v| (v - 15.0).abs() < 1e-5),
        "rank 0 should have 15.0 after AllReduce Avg, got {}",
        vals0[0]
    );
    assert!(
        vals1.iter().all(|&v| (v - 15.0).abs() < 1e-5),
        "rank 1 should have 15.0 after AllReduce Avg, got {}",
        vals1[0]
    );
}
...
fn test_nccl_rank_comm_on_stream() {
    ...
    let h0 = std::thread::spawn(move || {
        crate::tensor::set_current_gpu_device(0);
        comm0
            .all_reduce_on_stream(&[&t0c], ReduceOp::Sum, &stream0)
            .unwrap();
        stream0.synchronize().unwrap();
    });
    let h1 = std::thread::spawn(move || {
        crate::tensor::set_current_gpu_device(1);
        comm1
            .all_reduce_on_stream(&[&t1c], ReduceOp::Sum, &stream1)
            .unwrap();
        stream1.synchronize().unwrap();
    });
    h0.join().unwrap();
    ...
    let vals1 = t1.to_f32_vec().unwrap();
    assert!(
        vals0.iter().all(|&v| (v - 10.0).abs() < 1e-5),
        "rank 0 should have 10.0 after Sum, got {}",
        vals0[0]
    );
    assert!(
        vals1.iter().all(|&v| (v - 10.0).abs() < 1e-5),
        "rank 1 should have 10.0 after Sum, got {}",
        vals1[0]
    );
}
...
fn test_nccl_rank_comm_multi_tensor_batch() {
    ...
    assert!(
        va0.iter().all(|&v| (v - 2.0).abs() < 1e-5),
        "a0 should be 2.0"
    );
    assert!(
        vb0.iter().all(|&v| (v - 150.0).abs() < 1e-5),
        "b0 should be 150.0"
    );
    ...
    assert!(
        va1.iter().all(|&v| (v - 2.0).abs() < 1e-5),
        "a1 should be 2.0"
    );
    assert!(
        vb1.iter().all(|&v| (v - 150.0).abs() < 1e-5),
        "b1 should be 150.0"
    );
}
...
fn test_nccl_rank_comm_broadcast() {
    ...
    assert!(
        vals0.iter().all(|&v| (v - 42.0).abs() < 1e-5),
        "rank 0 (root) should retain 42.0, got {}",
        vals0[0]
    );
    assert!(
        vals1.iter().all(|&v| (v - 42.0).abs() < 1e-5),
        "rank 1 should receive 42.0 from root, got {}",
        vals1[0]
    );
}
...
fn test_nccl_three_gpu_all_reduce_sum() {
    ...
    let _lock = NCCL_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let comms = NcclComms::new(&[Device::CUDA(0), Device::CUDA(1), Device::CUDA(2)]).unwrap();
    ...
    for (i, t) in [&t0, &t1, &t2].iter().enumerate() {
        ...
        assert!(
            vals.iter().all(|&v| (v - 6.0).abs() < 1e-5),
            "rank {i} should have 6.0 after AllReduce Sum, got {}",
            vals[0]
        );
    }
}
