...
fn test_empty_dataset_errors() {
    ...
    impl DataSet for Empty {
        ...
        fn get(&self, _: usize) -> Result<Vec<Tensor>> {
            unreachable!()
        }
    }

    let result = DataLoader::from_dataset(Empty).batch_size(10).build();
...
fn test_zero_batch_size_errors() {
    ...
    let result = DataLoader::from_dataset(data).batch_size(0).build();
    assert!(result.is_err());
}
...
fn test_dataset_panic_is_an_err_batch_not_a_dead_loader() {
    ...
    assert!(
        first.is_ok(),
        "worker must survive a dataset panic: {:?}",
        first.err()
    );
}
...
fn test_dataset_err_mid_epoch_is_an_err_batch_not_a_dead_loader() {
    ...
    assert!(
        first.is_ok(),
        "worker must survive a dataset Err: {:?}",
        first.err()
    );
}
...
fn test_set_data_loader_rejected_while_epoch_iterator_active() {
    ...
    assert!(
        err.unwrap_err()
            .to_string()
            .contains("epoch iterator is active"),
...
fn test_scatter_fallback_without_data_loader() {
    ...
    let x =
        crate::autograd::Variable::new(Tensor::randn(&[3, 4], Default::default()).unwrap(), false);
...
fn test_transform_keys_views_and_reproduces() {
    ...
    let build = || {
        DataLoader::from_dataset(SequentialData { n: 8 })
            ...
            .augment(2)
            .transform(|rows, keys| {
                let offs: Vec<f32> = keys.iter().map(|k| k.repeat as f32 * 100.0).collect();
                let o = Tensor::from_f32(&offs, &[offs.len() as i64, 1], Device::CPU)?;
                Ok(vec![rows[0].add(&o)?])
            })
            .build()
            .unwrap()
...
fn test_el_che_clamping_proportional() {
    ...
    let mut clamped: Vec<usize> = counts
        .iter()
...
fn test_retry_on_oom_policy() {
    use crate::data::prefetch::{OOM_RETRY_ATTEMPTS, retry_on_oom};
...
fn test_vram_pool_mixed_assembly_preserves_batch_content() {
    ...
    assert_eq!(
        got.concat(),
        evens.iter().map(|&i| i as i64).collect::<Vec<_>>()
    );
    ...
    assert_eq!(
        got.concat(),
        mixed.iter().map(|&i| i as i64).collect::<Vec<_>>()
    );
    ...
    assert_eq!(
        got.concat(),
        all.iter().map(|&i| i as i64).collect::<Vec<_>>()
    );
}
...
fn test_sample_cache_content_identical_across_epochs() {
    ...
    let calls = std::sync::Arc::new(AtomicUsize::new(0));
    let mut loader = DataLoader::from_dataset(CountingData { n: 8, calls })
        .batch_size(4)
        .streaming()
        .sampler(Box::new(SequentialSampler::new(8)))
        .build()
...
fn test_disk_stage_errors_without_sample_layer() {
    ...
    assert!(
        err.to_string()
            .contains("disk_stage requires the sample layer")
    );
