...
- **Dormant until advised**: the tier has budget 0 (pure pass-through, one atomic load per row) until the first advisory arrives, so single-device runs, tests, and non-progressive modes never pay for it. The initial budget is conservative — half the host's RAM headroom split by world size — until consumption-proportional shares land.
- Staged content is reshuffle-invariant (sample-keyed): whatever the next epoch's permutation shares with current holdings is already staged.
