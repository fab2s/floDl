...
- **Sync-window RAM peak cuts on the CPU reduce path** — three model-sized transients no longer overlap at the reduce barrier (where every rank spikes simultaneously): the pre-scaled contribution scratch is freed the moment the wire frame is encoded (`all_reduce_weighted_owned`), the sent frame is dropped before the blocking reply read, and the reply decodes blob-direct into tensors (no intermediate `Vec<f32>` model copy). Found chasing the first-sync OOM on a RAM-tight two-rank host; applies to f32 and bf16 wires alike.
