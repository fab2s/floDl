...
- **`fdl libtorch download --rocm 7.0`.** `--path`, `--no-activate` and `--dry-run` are now listed in `fdl libtorch -h`.
- **`fdl probe` and `fdl setup` report missing prerequisites with the command to install them**: host tools (curl or wget, unzip, a C++ compiler) and the vendor toolkit headers for the active libtorch variant. Setup reports the native prerequisites only when the build is native.
...
- **Integrated GPUs no longer double-count memory.** An APU's aperture is carved out of host RAM, so the data plane reserves it once, subtracting unrealized headroom only. `gpu_ram_share` on `DataLoaderBuilder`, `TrainerConfig` and `DdpRunConfig` overrides the fraction.
- **The system ROCm runtime precedes libtorch's bundled copy on `LD_LIBRARY_PATH`** in cluster pre-build, remote dispatch and the container images.
...
Two public items in `flodl::monitor::record` changed shape. Both are
consequences of the interval/cadence work described under **Added** below, and
both are compile-time errors rather than silent behavior changes. Nothing else
in the crate's public surface was removed or re-signatured.
...
- **Dashboard: children are linked under the breadcrumb, and `sweep.sh` saves every cell's portal archive** — each level's children render as links beside the breadcrumb so a cluster run is browsable without going through the compare table, and the bench sweep passes `--save-dashboard` per cell so a full sweep leaves one self-contained portal archive per (model, mode) alongside the timeline artifacts. — each level's children render as links beside the breadcrumb so a cluster run is browsable without going through the compare table, and the bench sweep passes `--save-dashboard` per cell so a full sweep leaves one self-contained portal archive per (model, mode) alongside the timeline artifacts.
