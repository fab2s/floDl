...
- **Streaming wire encode with fused sender-side scaling (rank up-leg)** — a rank's reduce contribution now streams straight from its parameter staging to the socket, one tensor at a time: the frame length is computed exactly from shapes + dtype and committed as the length prefix up front, payload bytes are MAC'd as they are written, and the realized-work γ-scale runs inside the encode at byte level (bit-identical to the `mul_scalar` it replaces — asserted in tests). The send side of a sync window no longer materializes the scaled model copy, the frame's payload bytes, or the serialized body (which was buffered TWICE before): the peak send-side transient drops from ~4 model copies to ONE tensor's wire bytes. At OLMo-190M on the bf16 wire that is ~1.5GB less transient RAM per rank at the barrier, on top of the cuts above.
