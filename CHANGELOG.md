...
- **Sizing is automatic and conservative**: dormant until the prefetch governor's honest post-first-step probe, then one budget decision from measured free VRAM minus the governor's full in-flight reserve minus a safety margin. No fraction knob; **`DataLoaderBuilder::vram_pool(false)`** is the off switch.
