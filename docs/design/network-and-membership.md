...
2. **`state.json`** served by the dashboard HTTP server the
   controller already hosts - the dashboard's membership panel reads
   the same source.
...
costs a few lines more and gives the dashboard and the CLI the same
truth for free.
